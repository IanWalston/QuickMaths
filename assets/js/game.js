...
let game = {
  ...
  new_question: () => {
    ...
    let form = $("<input>")
      .attr("type", "text")
      .attr("id", "player_answer");
    let btn = $("<button>")
      .html("submit")
      .on("click", game.submit_answer);
    adiv
      .append(form)
      .append("<br>")
      .append(btn);
...
const initiate = () => {
  ...
  p.append(
    "Use the enter key to submit answers. All answers must be in decimal form."
  );
  p.append("<br>");
  p.append(
    "Get 6 right in 63 seconds to win, get 3 wrong to lose. Questions get harder after every 2 you get right."
  );
  p.append("<br>");
  p.append(
    "The answers to the questions will be shown after you complete the problem set."
  );
  p.append("<br>");
  p.append("<br>");
